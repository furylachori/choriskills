#!/usr/bin/env python3
"""
Bailian Qwen Image API Client

Generate images using the Bailian Qwen model.

Usage:
    python bailian_qwen_image.py generate --prompt "A serene mountain landscape at sunset, photorealistic"

Environment:
    BAILIAN_QWEN_API_KEY - Required. Your Bailian API key.
    BAILIAN_QWEN_API_BASE - Optional. API base URL (default: https://token-plan.ap-southeast-1.maas.aliyuncs.com)
"""
...
ALLOWED_SIZES = {"2048*2048", "1536*1536", "1024*1024"}

ALLOWED_HOST = "token-plan.ap-southeast-1.maas.aliyuncs.com"
ALLOWED_HOST_SUFFIXES = {".aliyuncs.com"}
_ALLOWED_ENV_KEYS = {"BAILIAN_QWEN_API_KEY", "BAILIAN_QWEN_API_BASE", "OUTPUT_DIR"}

_script_dir = os.path.dirname(os.path.abspath(__file__))
_QWEN_DEFAULT_API_BASE = "https://token-plan.ap-southeast-1.maas.aliyuncs.com"
...
def get_output_path(prompt=None, extension="png"):
    ...
    if prompt:
        ...
        filename = f"bailian_qwen_{slug}_{timestamp}_{os.getpid()}.{extension}"
    else:
        filename = f"bailian_qwen_{timestamp}_{os.getpid()}.{extension}"
...
def validate_size(size_str):
    """Validate --size format for Qwen model."""
    if size_str not in ALLOWED_SIZES:
        print(f"Error: --size must be one of {sorted(ALLOWED_SIZES)} for {MODEL} (got '{size_str}')", file=sys.stderr)
        sys.exit(EXIT_INPUT_ERROR)
...
def call_api(endpoint, data=None, timeout=HTTP_TIMEOUT):
    ...
    api_key = os.environ.get("BAILIAN_QWEN_API_KEY")
    if not api_key:
        print("Error: BAILIAN_QWEN_API_KEY environment variable is not set.", file=sys.stderr)
        sys.exit(EXIT_AUTH_ERROR)

    api_base = os.environ.get("BAILIAN_QWEN_API_BASE", _QWEN_DEFAULT_API_BASE)
    ...
    try:
        ...
        if e.code == 401 or e.code == 403:
            print(f"Error: Authentication failed. Check your BAILIAN_QWEN_API_KEY.", file=sys.stderr)
            sys.exit(EXIT_AUTH_ERROR)
        if e.code == 429:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Bailian Qwen Image API Client")
