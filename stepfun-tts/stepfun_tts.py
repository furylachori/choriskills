...
Environment:
    ...
    OUTPUT_DIR - Optional. Output directory (default: $HOME/.zeroclaw/agents/default/workspace/output)
"""
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if e.code in (429, 500, 502, 503, 504) and attempt < max_retries:
                ...
                try:
                    delay = min(float(ra), 60)  # Cap at 60 seconds
                except ValueError:
                    pass
    ...
    try:
        ...
        for family, _, _, _, sockaddr in addrinfos:
            ...
            if ip.is_private or ip.is_loopback or ip.is_link_local or ip.is_reserved:
                print(f"Error: SSRF check failed - hostname '{hostname}' resolves to private IP {ip}", file=sys.stderr)
                sys.exit(EXIT_INPUT_ERROR)
...
def _print_audio_metadata(path):
    """Print audio file metadata to stdout."""
...
def text_to_speech(args):
    ...
    # Validate length before sanitizing (sanitize_text truncates)
    ...
    try:
        with _urlopen_with_retry(req, timeout=args.timeout) as response:
            ...
            download_url(url, output_path, timeout=args.timeout)

            print(output_path)
            _print_audio_metadata(output_path)
            if args.verbose:
                if 'voice' in data:
        ...
        print(f"TIMEOUT: Request timed out after {args.timeout}s. Try again with --timeout {args.timeout*2}", file=sys.stderr)
        sys.exit(EXIT_NETWORK_ERROR)
    except urllib.error.HTTPError as e:
        error_body = e.read().decode()
        if len(error_body) > 200:
