...
def _print_video_metadata(path):
    """Print video file metadata to stderr."""
    ext = os.path.splitext(path)[1].lstrip('.').upper()
    print(f"Format: {ext}", file=sys.stderr)
    try:
        print(f"Size: {os.path.getsize(path)} bytes", file=sys.stderr)
    except OSError:
        pass
...
def validate_url_safe(url):
    ...
    if not hostname:
        ...
        sys.exit(EXIT_INPUT_ERROR)
    if hostname not in ALLOWED_HOSTS and not any(hostname == suffix or hostname.endswith("." + suffix) for suffix in ALLOWED_HOST_SUFFIXES):
...
def download_url(url, output_path, timeout=HTTP_TIMEOUT):
...
def call_api(endpoint, data=None, method="POST", timeout=HTTP_TIMEOUT):
    ...
    try:
        response = _urlopen_with_retry(req, timeout=timeout)
        try:
...
def query_task(task_id, timeout=HTTP_TIMEOUT):
...
def get_file_download_url(file_id, timeout=HTTP_TIMEOUT):
...
def poll_task(task_id, args):
    ...
    for attempt in range(POLL_MAX_ATTEMPTS):
        result = query_task(task_id, timeout=args.timeout)
...
def generate_video(args):
    ...
    response = call_api("video_generation", data=data, timeout=args.timeout)
    ...
    download_url_result = get_file_download_url(file_id, timeout=args.timeout)
    ...
    download_url(download_url_result, output_path, timeout=args.timeout)

    print(output_path)
    _print_video_metadata(output_path)
...
def retrieve_video(args):
    ...
    result = query_task(args.task_id, timeout=args.timeout)
...
def main():
    ...
    ret_parser.add_argument("--timeout", type=int, default=120, help="HTTP timeout in seconds (default: 120)")
