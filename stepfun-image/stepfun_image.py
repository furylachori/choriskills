...
def call_api(endpoint, data=None, files=None, timeout=HTTP_TIMEOUT):
...
def generate_image(args):
    ...
    response = call_api("images/generations", data=data, timeout=args.timeout)
...
def edit_image(args):
    ...
    response = call_api("images/edits", data=data, files={"image": args.input}, timeout=args.timeout)
    ...
    download_url(image_info["url"], args.output, timeout=args.timeout)
...
def main():
    ...
    edit_parser.add_argument("--timeout", type=int, default=120, help="HTTP timeout in seconds (default: 120)")
